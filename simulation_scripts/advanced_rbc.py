...
class AdvancedRBC(object):

    def __init__(self, ts, doy, month, 
                 pv_enabled, wind_enabled, day_counter,
        ...
        if self.ts == 0:
            ...
            self.sh_enabled = sh_activation_status(month, day_counter)
    ...
    def dhw_charging(self, dhw_tank_T, cyc_dhw):
        ...
        if (self.pv_enabled or self.wind_enabled) and dhw_tank_T[0] < self.dhw_lim_T:
    ...
    def sh_charging(self, sh_tank_T, cyc_sh, charge_dhw):
        ...
        sh_enabled = self.sh_enabled
        pv_enabled = self.pv_enabled
        wind_enabled = self.wind_enabled
        
        if sh_enabled is False:
            cyc_sh = 0
        
        # Force SH charge when PV or wind exceeds threshold and top node is below limit temp
        elif ((pv_enabled or wind_enabled) 
              and charge_dhw == 0 and sh_tank_T[0] < self.sh_lim_T) and sh_enabled:
        ...
        # Force SH charge when below minimum temp, or previously charging
              ...
              and charge_dhw == 0 and sh_tank_T[0] < self.sh_max_T) and sh_enabled:
